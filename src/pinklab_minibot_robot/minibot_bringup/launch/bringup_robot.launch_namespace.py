...
def generate_launch_description():
    ...
    load_minibot_io_controller = Node(
        package='controller_manager',
        executable='spawner',
        namespace=namespace,
        arguments=['minibot_io_controller', '-c', 'controller_manager'],
        output='screen'
    )
    ...
    handler1 = RegisterEventHandler(
        event_handler=OnProcessStart(
            target_action=control_node, on_start=[load_joint_state_broadcaster],
            )
        )

    handler2 = RegisterEventHandler(
        event_handler=OnProcessExit(
            target_action=load_joint_state_broadcaster, on_exit=[load_base_controller],
            )
        )
    handler3 = RegisterEventHandler(
        event_handler=OnProcessExit(
            target_action=load_base_controller, on_exit=[load_minibot_io_controller],
            )
        )
